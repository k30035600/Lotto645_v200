...
def fetch_latest_lotto(force=False):
    ...
    3. 오늘(KST)이 추첨일(토요일)이고 20:25 KST 이후면 API 호출 시도(당첨 방송 ~20:30 반영).
            ...
            if last_date_str:
                ...
                if last_date >= today_kst:
                    ...
                    if today_kst < next_draw_date:
                        ...
                        # 오늘이 추첨일(토요일). ~20:30 방송 직후 반영을 위해 20:25 KST 이후 API 호출 시도
                        if now_kst.hour > 20 or (now_kst.hour == 20 and now_kst.minute >= 25):
                            need_api_call = True
                        else:
                            need_api_call = False
                    else:
